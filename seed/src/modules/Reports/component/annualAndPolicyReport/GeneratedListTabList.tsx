...
import { getListOfAnnualPolicy } from '../../api/reportsApi';
import { Tpager } from '../../model/reportsModel';

const GeneratedListTabList = () => {
  ...
  const fetchAllGeneratedReportsList = async (params) => {
    const res = await getListOfAnnualPolicy(params);
  ...
  return (
    <Box sx={{ mt: 6 }}>
      <Tables
        rows={allReportsList || []}
        className={'reportsTableList withGeneratedTable GapAssessmentList'}
        columns={columns}
        pager={pager}
...
export default GeneratedListTabList;
