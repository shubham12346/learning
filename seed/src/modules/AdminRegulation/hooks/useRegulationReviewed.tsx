...
import {
  ...
  updateOrSubmitAParticularReviewedRegulation
...
const useRegulationReviewed = () => {
  ...
  const handleSubmit = async (regulationDetail, isSubmit) => {
    ...
    try {
      const res = await updateOrSubmitAParticularReviewedRegulation(
        payloadObject
      );
      if (
        isSubmit ||
      ...
      if (res.status === '200' || '201') {
        ...
        if (!isSubmit) {
          setIsRegulationUpdate({
            ...
            regulatoryOrganizationId:
              regulationDetail?.regulatoryOrganizationId,
            organizationName: regulationDetail?.regulatoryBody
          });
          setButtons((prev) => ({
            ...prev,
...
export default useRegulationReviewed;
