...
const EvaluateRegulation = (tabIndex) => {
  ...
  // states
  const [selectedRegulationType, setSelectedRegulationType] = useState('all');
  ...
  const [scrollToSelectedItem, setScrollToSelectedItem] = useState('');
  ...
  const {
    ...
    APIEndPoint.adminRegulation.searchRegulation,
  ...
  const fetchAllInitialStates = () => {
    ...
    dispatch(fetchRegulationTypeList());
    dispatch(fetchOrganizationDropdownOptions());
  };

  const handleRegulationType = (e) => {
    setSelectedRegulationType(e.target.value);
    dispatch(fetchRegulationList(e.target.value));
    dispatch(fetchRegOrganizationListWithCount(e.target.value));
  };
  ...
  useEffect(() => {
    if (
      ...
      regulationListBasedOnOrganization?.length > 0
    )
      setTimeout(() => {
        setScrollToSelectedItem(regulationDetail?.regulationId);
      }, 1000);
  }, [regulationListBasedOnOrganization, regulationDetail]);
  ...
  return (
    <Box className=" ">
      <Box className="flex-basic-start width-search-input-box">
        ...
        <Box sx={{ mx: 10 }} className="filterSelect">
          <Select
            label={''}
            placeholder="Regulation Type"
            value={selectedRegulationType || 'all'}
            options={regulationTypeDropdown || []}
            itemValue={'name'}
            itemText={'displayName'}
            onChange={(e) => handleRegulationType(e)}
          />
        </Box>
      </Box>
      <Box sx={{ mt: 6 }}>
              ...
              <Box>
                {!noRegulation && (
                  <Box className="regulationListScroll">
                    {searchedRegulation && searchSelected ? (
                      ...
                      <RegulationsList
                        ...
                        accordionListActiveItem={
                          scrollToSelectedItem || regulationDetail?.regulationId
                        }
          ...
          <Grid item xs={12} sm={12} md={12} lg={9} xl={9}>
            {noRegulation ? (
              ...
              <RegulationDetailCard
                ...
                selectedRegulationType={selectedRegulationType}
...
export default EvaluateRegulation;
